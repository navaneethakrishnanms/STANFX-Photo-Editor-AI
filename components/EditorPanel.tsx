...
const TextEraserIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 7V5h16v2"/><path d="M12 19v-8"/><path d="M7 19h10"/><path d="M9 11h6"/></svg>;
...
type Tool = 'blur' | 'grayscale' | 'brightness' | 'contrast' | 'saturate' | 'temperature' | 'tint' | 'vignette' | 'noise' | 'rotate' | 'crop' | 'text' | 'shapes' | 'draw' | 'erase' | 'overlay' | null;
...
const toolset = [
    ...
    { tool: 'erase' as Tool, icon: EraserIcon, tooltip: 'Erase Lines' },
...
// Optimized drawing functions with performance improvements
const drawPath = (ctx: CanvasRenderingContext2D, path: Path) => {
    ...
    // Use moveTo for the first point
...
const drawShape = (ctx: CanvasRenderingContext2D, shape: ShapeObject) => {
    ...
    switch (shape.type) {
        ...
        case 'triangle':
            ...
            break;
        case 'star':
            const starCenterX = shape.x + shape.w / 2;
            const starCenterY = shape.y + shape.h / 2;
...
const renderCanvas = (
    ...
    // 1. Apply view transformations (pan & zoom) - use single save/restore
    ctx.save();
    ctx.translate(pan.x, pan.y);
    ctx.scale(zoom, zoom);
    
    // 2. Build filter string once
    const filterString = `blur(${blurLevel}px) grayscale(${grayscaleLevel}%) brightness(${brightness}%) contrast(${contrast}%) saturate(${saturate}%)`;
    
    // 3. Draw base image with filters
    ...
    // 4. Temperature/Tint overlays (optimized with single save/restore)
    ...
    // 5. Draw persistent items (optimized iteration)
    if (paths.length > 0) {
    ...
    // 7. Draw Overlay Image (optimized with offscreen canvas reuse)
    if (overlay) {
    ...
    // 8. Restore from view transformations
    ctx.restore();

    // 9. Draw screen-space effects (not affected by zoom/pan)
    if (vignette > 0) {
        ctx.save();
        const centerX = canvas.width / 2;
        const centerY = canvas.height / 2;
        const innerRadius = canvas.width / 3;
    ...
    if (noise > 0) {
        ...
        // Optimized noise generation with stride
    ...
    // 10. Crop rectangle UI (only drawn for the visible canvas)
    if (interactiveState.cropRectForUI) {
        ctx.save();
        const cropRect = interactiveState.cropRectForUI;
        
        // Normalize crop rectangle
        let rectX = cropRect.x;
        let rectY = cropRect.y;
        let rectW = cropRect.w;
        let rectH = cropRect.h;
        ...
        // Only show crop UI if dimensions are reasonable
        const minSize = 5; // Minimum 5 pixels
        if (uiRect.w > minSize && uiRect.h > minSize) {
            ...
            // Draw crop border with grid lines for better alignment
            ...
            // Vertical lines
            ctx.beginPath();
            ctx.moveTo(uiRect.x + uiRect.w / 3, uiRect.y);
            ctx.lineTo(uiRect.x + uiRect.w / 3, uiRect.y + uiRect.h);
            ctx.moveTo(uiRect.x + (2 * uiRect.w) / 3, uiRect.y);
            ctx.lineTo(uiRect.x + (2 * uiRect.w) / 3, uiRect.y + uiRect.h);
            
            // Horizontal lines
            ctx.moveTo(uiRect.x, uiRect.y + uiRect.h / 3);
            ctx.lineTo(uiRect.x + uiRect.w, uiRect.y + uiRect.h / 3);
            ctx.moveTo(uiRect.x, uiRect.y + (2 * uiRect.h) / 3);
            ctx.lineTo(uiRect.x + uiRect.w, uiRect.y + (2 * uiRect.h) / 3);
            ctx.stroke();
...
export const EditorPanel: React.FC<EditorPanelProps> = ({
    ...
    const pushToHistory = useCallback((newState: CanvasState) => {
        ...
        setHistoryIndex(prevIndex => {
            ...
            // Return new index (length - 1 after adding the new entry)
            return prevIndex + 1;
        });
    ...
    const applyNewBaseImage = useCallback((newBase64: string) => {
        ...
        img.onload = () => {
            ...
            setHistoryIndex(prevIndex => {
                setHistory(prev => {
                    // Remove future history and add new entry
    ...
    const handleUndo = useCallback(() => {
        setHistoryIndex(prevIndex => {
            if (prevIndex > 0) {
                ...
                // Access history using a ref or pass through setHistory to avoid closure
                setHistory(prevHistory => {
                    if (prevHistory[newIndex]) {
                        loadState(prevHistory[newIndex]);
                    }
                    return prevHistory;
                });
                return newIndex;
    ...
    // Optimized rendering effect with proper dependency management
    useEffect(() => {
    ...
    const handleApplyCrop = useCallback(() => {
        ...
        renderCanvas(offscreenCtx, offscreenCanvas, baseImage, canvasState, 1, {x:0, y:0}, { 
            cropRectForUI: null, 
            drawingShape: null, 
            drawingPath: null, 
            drawingMaskPath: null 
        });

        // Normalize the crop rectangle (handle negative width/height from any drag direction)
        let rectX = cropRect.x;
        let rectY = cropRect.y;
        let rectW = cropRect.w;
        let rectH = cropRect.h;
        
        // Handle dragging in any direction (like phone gallery)
        if (rectW < 0) {
            rectX = rectX + rectW;
        ...
        // Round to whole pixels for cleaner crops
        ...
        rectX = Math.max(0, Math.min(rectX, offscreenCanvas.width - 1));
        rectY = Math.max(0, Math.min(rectY, offscreenCanvas.height - 1));
        rectW = Math.max(1, Math.min(rectW, offscreenCanvas.width - rectX));
        rectH = Math.max(1, Math.min(rectH, offscreenCanvas.height - rectY));
        ...
        setCropRect(null);
        setActiveTool(null);
    }, [cropRect, baseImage, canvasState, mimeType, applyNewBaseImage]);

    // Optimized coordinate transformation using memoized calculations
    const getCanvasCoords = useCallback((e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
        ...
        // Using more precise calculation to avoid floating point errors
    ...
    const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
        ...
        if (activeTool === 'text') {
            const ctx = canvasRef.current!.getContext('2d')!;
            // Check from top to bottom (reverse order) to prioritize topmost text
            for (let i = canvasState.texts.length - 1; i >= 0; i--) {
                ...
                // Improved hit detection with proper bounding box
                if (x >= t.x && x <= t.x + textWidth && y <= t.y && y >= t.y - textHeight) {
                    // Store the exact offset from the text's position to maintain smooth dragging
        ...
        if (activeTool === 'erase') {
            ...
            const pathsToDelete: Set<number> = new Set(); // Use Set for O(1) lookup
            
            // Only check paths (drawn lines) for intersection, not shapes or text
            for (let i = 0; i < canvasState.paths.length; i++) {
                const path = canvasState.paths[i];
                for (let j = 0; j < path.points.length - 1; j++) {
                    const p1 = path.points[j];
                    const p2 = path.points[j + 1];
                    const dist = pointToLineDistance(x, y, p1.x, p1.y, p2.x, p2.y);
                    if (dist <= radius + path.size / 2) {
                        pathsToDelete.add(i);
                        break;
                    }
                }
            ...
            if (pathsToDelete.size > 0) {
                setCanvasState(prev => {
                    const newPaths = prev.paths.filter((_, idx) => !pathsToDelete.has(idx));
                    const newState = { ...prev, paths: newPaths };
                    pushToHistory(newState);
                    return newState;
                });
            }
    ...
    const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
        ...
        // Optimized text dragging with smooth position updates
        ...
        if (isDrawing && startPoint.current) {
            if (activeTool === 'draw' && drawingPath) {
                // Throttle point collection for smoother performance
                const lastPoint = drawingPath.points[drawingPath.points.length - 1];
                const dx = x - lastPoint.x;
                const dy = y - lastPoint.y;
                const distance = Math.sqrt(dx * dx + dy * dy);
                
                // Only add point if it's far enough from the last point (reduces memory usage)
                if (distance > 2) {
                    setDrawingPath(prev => prev ? { ...prev, points: [...prev.points, { x, y }] } : null);
                }
            } else if (activeTool === 'crop') {
                // Improved crop with proper rectangle calculation
                ...
                // Fixed shape dragging with correct position tracking
                const { x: startX, y: startY } = startPoint.current;
                ...
                if (overlayToolMode === 'erase' && drawingMaskPath) {
                    // Throttle eraser path points
                    const lastPoint = drawingMaskPath.points[drawingMaskPath.points.length - 1];
                    const dx = x - lastPoint.x;
                    const dy = y - lastPoint.y;
                    const distance = Math.sqrt(dx * dx + dy * dy);
                ...
                } else if (overlayToolMode === 'move' && startPoint.current) {
                ...
                // Optimized eraser with Set-based deletion
                const radius = eraserSize / 2;
                const pathsToDelete: Set<number> = new Set();
                
                // Only check paths (drawn lines) for intersection
                for (let i = 0; i < canvasState.paths.length; i++) {
                    const path = canvasState.paths[i];
                    for (let j = 0; j < path.points.length - 1; j++) {
                        const p1 = path.points[j];
    ...
    const handleApplyAIEdit = useCallback(() => {
        ...
        renderCanvas(tempCtx, tempCanvas, baseImage, canvasState, 1, {x:0, y:0}, { 
            cropRectForUI: null, 
            drawingShape: null, 
            drawingPath: null, 
            drawingMaskPath: null 
    ...
    }, [baseImage, canvasState, mimeType, onAIPromptEdit]);
    ...
    }, [baseImage, canvasState, mimeType, onSaveToGallery]);
    ...
    const handleDownload = useCallback(() => {
        ...
        link.click();
    }, [baseImage, canvasState, mimeType]);
    ...
    const handleOverlayUpload = useCallback((file: File) => {
        const reader = new FileReader();
        reader.onload = e => {
            const img = new Image();
            img.onload = () => {
                setCanvasState(prev => {
                    if (!canvasRef.current || !baseImage) return prev;
                    ...
                    const overlayX = (canvasWidth - img.width) / 2;
                    const overlayY = (canvasHeight - img.height) / 2;
    ...
    const handleApplyOverlay = useCallback(() => {
        ...
        setActiveTool(null);
    }, [baseImage, canvasState, mimeType, applyNewBaseImage]);

    const handleDiscardOverlay = useCallback(() => {
        setCanvasState(prev => {
            ...
            return (
                <div className={containerClasses}>
                    <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted">Brush over drawn lines to remove them. Only drawn lines will be affected.</p>
                    </div>
                </div>
            );
        case 'overlay':
