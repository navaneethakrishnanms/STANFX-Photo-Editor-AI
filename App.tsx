...
const App: React.FC = () => {
  ...
  const handleDeleteFromGallery = useCallback((imageId: string) => {
    if (!currentUsername) return;
    setAllGalleries(prev => {
      const currentUserGallery = prev[currentUsername] || [];
      const updatedGallery = currentUserGallery.filter(img => img.id !== imageId);
      return {
        ...prev,
        [currentUsername]: updatedGallery
      };
    });
  }, [currentUsername]);
           ...
           <AdminPanel allUsers={allUsers} setAllUsers={setAllUsers} currentUser={currentUser} onBack={() => setAppState('select')} />
            ...
            <>
              ...
              <div className="grid grid-cols-1 gap-12">
                  ...
                  <Gallery images={galleryImages} onDeleteImage={handleDeleteFromGallery} />
              </div>
            </>
        )}
      </main>
